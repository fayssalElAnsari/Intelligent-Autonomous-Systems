...
static void initialize() {
  ...
  if (strncmp(robot_name, "e-puck", 6) == 0) {
    ...
    const double khepera3_speed_unit = 0.0006;

    // this is the condition being met for now!
    num_sensors = 9;
...
// question 6: avoid an obstacle to the right!
const double khepera3_avoid_matrix[9][2] = {
    {-5000, -5000}, {-20000, 40000}, {-30000, 50000}, {50000, -40000},
    {70000, -60000}, {50000, -40000}, {40000, -20000}, {-5000, -5000},
    {-10000, -10000}
};

int main() {
  // if obstacle encountered simply stop
  if (ALGO == 1){
    printf("Applying algo 1\n");

    double obstacle_threshold = 20; // Set a suitable threshold for obstacle detection
    initialize();
    ...
    while (wb_robot_step(time_step) != -1) {
      ...
      // only sensors from 2 to 7
      for (i = 2; i < 6; i++) {
        sensors_value[i] = wb_distance_sensor_get_value(sensors[i]);
        printf("Sensor %d value: %lf\n", i, sensors_value[i]);
        fflush(stdout); // Flush the output buffer

        if (sensors_value[i] > obstacle_threshold) {
          obstacle_detected = 1;
          break;
        }
      }
  ...
  // follow obstacle delimiter counter-clockwise
  else {
    
    // should be modified (after training)
    const double circular_navigation_matrix[9][2] = {
      {5000, -5000},   {10000, -20000},  {15000, -30000}, {20000, -40000},
      {-40000, 20000}, {-30000, 15000},  {-20000, 10000}, {-10000, 5000},
      {0, 0}
    };
  
    printf("Applying algo 3\n");
    initialize();
      ...
      while (wb_robot_step(time_step) != -1) {
        ...
        // Check for obstacles and set obstacle_detected flag
        ...
        /*
         * Modify the Braitenberg algorithm to navigate around an obstacle in circles.
         */
        for (i = 0; i < 2; i++) {
          speed[i] = 0.0;
          ...
          for (j = 0; j < num_sensors; j++) {
            if (obstacle_detected) {
              // Adjust weights to circle around the obstacle
              speed[i] += speed_unit * circular_navigation_matrix[j][i] * (1.0 - (sensors_value[j] / range));
            } else {
              // Normal behavior
              speed[i] += speed_unit * matrix[j][i] * (1.0 - (sensors_value[j] / range));
            }
